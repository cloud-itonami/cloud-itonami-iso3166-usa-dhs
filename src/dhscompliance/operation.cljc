(ns dhscompliance.operation
  "OperationActor -- one DHS compliance operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (DHSCompliance-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the DHS Federal-Procurement /
  Supply-Chain-Risk Compliance Governor (:govern) and the rollout
  ...
  One graph run = one compliance operation (cisa-scrm / fasc-exclusion
  / hsar-compliance track). No unbounded inner loop.
  ...
  governor is clean -- see `dhscompliance.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [dhscompliance.dhscompliancellm :as dhscompliancellm]
            [dhscompliance.governor :as governor]
            [dhscompliance.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (dhscompliancellm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (dhscompliancellm/-advise advisor store request)]
            {:proposal p :audit [(dhscompliancellm/trace request p)]})))

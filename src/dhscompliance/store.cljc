(ns dhscompliance.store
  "SSoT for the USA-DHS (Department of Homeland Security) compliance
  ...
  (test/dhscompliance/store_contract_test.clj).

  The primary entity here is an `engagement` -- one operator's
  compliance engagement. The catalog has FOUR tracks
  (`dhscompliance.facts`), but only `:cisa-scrm` (Section 889
  covered-vendor procurement screening) is a real filing/draft +
  filing/submit actuation track (`dhscompliance.facts/filing-tracks`)
  -- `:fasc-exclusion` and `:hsar-compliance` are assess-only (there is
  no 'submit a filing' action for either; the governor's
  `filing-op-on-non-filing-track` check enforces this), so this store
  only tracks draft/submit state for `:cisa-scrm`.

  plus engagement-level gating facts grounded in `dhscompliance.facts`:
    :vendor-screened-against-covered-list? -- independently-verified
                                   Section 889 5-vendor screening,
                                   gates `:cisa-scrm` `:filing/submit`.
    :cfats-relevant? / :cfats-status-verified? -- the CFATS live-
                                   status-check discipline from
                                   `dhscompliance.facts`'s
                                   `:cfats-status` disambiguation entry,
                                   gates `:cisa-scrm` `:filing/draft`/
                                   `:filing/submit`.

  filing-draft and filing-submit actuation events apply to the
  `:cisa-scrm` track on the SAME engagement record (draft first,
  submit later). Dedicated double-actuation-guard booleans
  (`:cisa-scrm-drafted?`/`:cisa-scrm-submitted?`, never a single
  `:status` value).

  The ledger stays append-only on every backend."
  (:require [dhscompliance.registry :as registry]
            [langchain.db :as d]
...
(def ^:private track-fields
  {:cisa-scrm {:drafted? :cisa-scrm-drafted?     :draft-number :cisa-scrm-draft-number
               :submitted? :cisa-scrm-submitted? :submit-number :cisa-scrm-submit-number}})

;; ----------------------------- demo data -----------------------------

(defn demo-data
  "A small, self-contained engagement set covering both actuation
  lifecycles (draft, submit) on the `:cisa-scrm` filing track, plus the
  governor's own dossier-grounded checks: a clean case (eng-1, vendor
  screened clean, not CFATS-relevant), a fabrication-defense probe case
  (eng-2, used for the no-spec-basis / FASC-CISA-misattribution /
  HSAR-CISA-conflation request-level probes), a fee-mismatch case
  (eng-3), a vendor-screening-incomplete case (eng-4, `:cisa-scrm`
  track submitted before `:vendor-screened-against-covered-list?` is
  true), and a CFATS-status-unverified case (eng-5, `:cfats-relevant?
  true` with `:cfats-status-verified? false`)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Meridian Federal Solutions LLC" :portal "acquisition.gov / eCFR"
             :base-fee 620000 :monthly-rate 38000 :monitoring-months 12
             ...
             :status :intake}
    "eng-2" {:id "eng-2" :operator "Cascadia Federal Partners LLC" :portal "acquisition.gov / eCFR"
             :base-fee 620000 :monthly-rate 38000 :monitoring-months 12
             :audit-export? true :export-fee 120000 :claimed-fee 1196000.0
             :vendor-screened-against-covered-list? true
             :cfats-relevant? false :cfats-status-verified? false
             :cisa-scrm-drafted? false :cisa-scrm-submitted? false
             :status :intake}
    "eng-3" {:id "eng-3" :operator "Palisade Federal Systems LLC" :portal "acquisition.gov / eCFR"
             :base-fee 620000 :monthly-rate 38000 :monitoring-months 12
             ...
             :cisa-scrm-drafted? false :cisa-scrm-submitted? false
             :status :intake}
    "eng-4" {:id "eng-4" :operator "Redstone Federal Advisory LLC" :portal "acquisition.gov / eCFR"
             :base-fee 620000 :monthly-rate 38000 :monitoring-months 12
             :audit-export? false :export-fee nil :claimed-fee 1076000.0
             :vendor-screened-against-covered-list? false
             :cfats-relevant? false :cfats-status-verified? false
             :cisa-scrm-drafted? false :cisa-scrm-submitted? false
             :status :intake}
    "eng-5" {:id "eng-5" :operator "Beacon Federal Chemical Security LLC" :portal "acquisition.gov / eCFR"
             :base-fee 620000 :monthly-rate 38000 :monitoring-months 12
             :audit-export? false :export-fee nil :claimed-fee 1076000.0
             :vendor-screened-against-covered-list? true
             :cfats-relevant? true :cfats-status-verified? false
             :cisa-scrm-drafted? false :cisa-scrm-submitted? false
             :status :intake}}})
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months
                               ...
                               vendor-screened-against-covered-list?
                               cfats-relevant? cfats-status-verified?
                               cisa-scrm-drafted? cisa-scrm-draft-number
                               cisa-scrm-submitted? cisa-scrm-submit-number
                               status]}]
  (cond-> {:engagement/id id}
    ...
    (some? vendor-screened-against-covered-list?)  (assoc :engagement/vendor-screened-against-covered-list? vendor-screened-against-covered-list?)
    (some? cfats-relevant?)                        (assoc :engagement/cfats-relevant? cfats-relevant?)
    (some? cfats-status-verified?)                 (assoc :engagement/cfats-status-verified? cfats-status-verified?)
    (some? cisa-scrm-drafted?)                     (assoc :engagement/cisa-scrm-drafted? cisa-scrm-drafted?)
    cisa-scrm-draft-number                         (assoc :engagement/cisa-scrm-draft-number cisa-scrm-draft-number)
    (some? cisa-scrm-submitted?)                   (assoc :engagement/cisa-scrm-submitted? cisa-scrm-submitted?)
    cisa-scrm-submit-number                        (assoc :engagement/cisa-scrm-submit-number cisa-scrm-submit-number)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/vendor-screened-against-covered-list?
   :engagement/cfats-relevant? :engagement/cfats-status-verified?
   :engagement/cisa-scrm-drafted? :engagement/cisa-scrm-draft-number
   :engagement/cisa-scrm-submitted? :engagement/cisa-scrm-submit-number
   :engagement/status])
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :vendor-screened-against-covered-list? (boolean (:engagement/vendor-screened-against-covered-list? m))
     :cfats-relevant? (boolean (:engagement/cfats-relevant? m))
     :cfats-status-verified? (boolean (:engagement/cfats-status-verified? m))
     :cisa-scrm-drafted? (boolean (:engagement/cisa-scrm-drafted? m))
     :cisa-scrm-draft-number (:engagement/cisa-scrm-draft-number m)
     :cisa-scrm-submitted? (boolean (:engagement/cisa-scrm-submitted? m))
     :cisa-scrm-submit-number (:engagement/cisa-scrm-submit-number m)
